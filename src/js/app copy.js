...
inputText.addEventListener('keypress', (event) => {
  ...
  if (event.key === 'Enter') {
    ...
    //   },
    // }); // ----- Сервер отключается ----- // а теперь пробуем: stringify
    ...
    ); // ----- Сервер не отключается ----- // а теперь пробуем: stringify
...
ws.addEventListener('message', (e) => {
  ...
  // data.forEach(elem => { // for (let i = 1; i < arr.length; i++) {
  ...
  //   // перебираем всех подключенных пользователей
  //   console.log(elem);
  // }); // ошибка: Uncaught TypeError: data.forEach is not a function at WebSocket.
